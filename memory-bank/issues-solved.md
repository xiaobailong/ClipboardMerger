...
- 复发判据:
  ```bat
  findstr /n "%~f0" build.bat
  findstr /n "logpath" build.bat
  ```
  期望：第一条命中 `call "%~f0" %* 1>> "%_CM_LOGFILE%" 2>&1` 那一行；第二条 **0 命中**。
  出现 `2>&1 | powershell`、`-Command "... Out-File ..."` 或 `_logpath.tmp` 这类写法 ⇒ 复发。
...
- 首次记录: 2026-09-23 ／ 最近复核: 2026-09-23（静态判据通过）
